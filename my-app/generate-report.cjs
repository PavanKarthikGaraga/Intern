...
async function run() {
  ...
  const [[{ totalStudents }]] = await connection.query('SELECT COUNT(*) as totalStudents FROM registrations');
  // completed students = where pass = 1 ? let's assume where pass = 1 or something, or we can just say 0 for now as in the template
  ...
  const [slotRows] = await connection.query(`
    SELECT 
      slot, 
      COUNT(*) as total,
      SUM(CASE WHEN LOWER(TRIM(mode)) IN ('remote', 'hometown') THEN 1 ELSE 0 END) as remote,
      SUM(CASE WHEN LOWER(TRIM(mode)) IN ('incampus', 'in campus', 'in-campus') THEN 1 ELSE 0 END) as incampus,
      SUM(CASE WHEN LOWER(TRIM(mode)) IN ('invillage', 'in village', 'in-village') THEN 1 ELSE 0 END) as invillage
    FROM registrations
  ...
  for (const row of slotRows) {
    slotHtml += `
      <tr>
        ...
        <td style="border: 1px solid black; padding: 8px;">${row.remote}</td>
        <td style="border: 1px solid black; padding: 8px;">${row.incampus}</td>
        <td style="border: 1px solid black; padding: 8px;">${row.invillage}</td>
      </tr>
    `;
  }
  ...
  <body style="font-family: Arial, sans-serif; line-height: 1.6; padding: 40px; border: 2px solid #000; max-width: 800px; margin: 0 auto;">
  ...
  fs.writeFileSync('Social_Internship_Report_May_2026.doc', htmlContent);
  console.log('Report generated successfully as Social_Internship_Report_May_2026.doc');
